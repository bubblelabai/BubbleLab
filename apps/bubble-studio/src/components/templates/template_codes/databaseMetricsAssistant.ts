...
export class ChatWithYourDatabaseFlow extends BubbleFlow<'schedule/cron'> {
  ...
  async handle(payload: CustomCronPayload): Promise<Output> {
    ...
    // Examines the PostgreSQL database structure to discover all available tables,
    // columns, and their data types. This provides the AI agent with context about
    // what data is available to query. Parameters: dataSourceType ('postgresql'),
    // ignoreSSLErrors (true to handle SSL certificate issues), includeMetadata
    // (true to get detailed table/column information). This foundational step enables
    // the AI to generate accurate SQL queries based on the actual database structure.
    ...
    // Performs intelligent data analysis by exploring the database, running SQL queries,
    // and generating a comprehensive HTML report. This AI agent uses the sql-query-tool
    // to iteratively explore data, identify trends, calculate metrics, and create
    // actionable insights. Parameters: message (includes database schema and user query),
    // systemPrompt (defines the agent as a data analyst), model (gemini-2.5-pro with
    // jsonMode for structured output), tools (sql-query-tool for database access),
    // maxIterations (20 to allow thorough analysis). This is the core intelligence that
    // transforms raw database queries into meaningful business insights with visualizations.
    ...
    // HTML email. This makes the insights immediately accessible to the user in their
    // inbox. Parameters: operation ('send_email'), to (recipient email address),
    // subject (includes report title and date), html (the complete HTML report with
    // metrics, insights, and visualizations generated by the AI agent). This final
    // step completes the workflow by delivering actionable data insights in a convenient,
    // readable format.
