...
describe('BubbleFlow Validation', () => {
  ...
  describe('Invalid BubbleFlow validation', () => {
    ...
    it('should fail validation for unregistered bubble class', async () => {
      const code = getFixture('unregistered-bubble-flow');
      const result = await validateBubbleFlow(code);
      ...
      expect(
        result.errors?.some((error) =>
          error.includes('Unregistered bubble class: CustomUnregisteredBubble')
        )
      ).toBe(true);
    });
