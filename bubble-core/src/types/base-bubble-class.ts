...
> implements IBubble<TResult>
{
  ...
  constructor(params: unknown, context?: BubbleContext) {
    ...
    try {
      ...
      console.log('Current unique id:', context?.currentUniqueId);
      ...
      if (context && context.dependencyGraph && context.currentUniqueId) {
        console.log('Computing child context');
      ...
      throw new Error(`Parameter validation failed: ${String(error)}`);
    }
  }
    ...
    if (this.resultSchema) {
      try {
        ...
        if (!finalResult.success) {
          logger?.error(
            `[${this.name}] Execution did not succeed: ${finalResult.error}`
          );
        }
        ...
        logger?.error(
          `[${this.name}] Result validation failed: ${validationError instanceof Error ? validationError.message : 'Unknown validation error'}`
        );
        if (validationError instanceof Error) {
          logger?.error(
            `[${this.name}] Validation error details: ${validationError.message}`
          );
        }
        ...
        const errorResult = {
          success: false,
          data: {} as TResult,
          error: `Result validation failed: ${validationError instanceof Error ? validationError.message : 'Unknown validation error'}`,
          executionId: randomUUID(),
          timestamp: new Date(),
        };

        // Log bubble execution completion (with error)
        logger?.logBubbleExecutionComplete(
          this.context?.variableId ?? -999,
          this.name,
          this.name,
        ...
        // Logs the execution error and reason
        logger?.error(
          `[${this.name}] Execution error when validating result: ${validationError instanceof Error ? validationError.message : 'Unknown validation error'}`
        );
      ...
      logger?.error(
        `[${this.name}] No result schema defined for bubble ` + this.name
      );
    }
